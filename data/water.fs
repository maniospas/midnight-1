...
void main()
{
    ...
    // Rotate the sample point, then tile
    float c = cos(flowAngle), s = sin(flowAngle);
    vec2 rotated = vec2(tileUV.x * c - tileUV.y * s,
                        tileUV.x * s + tileUV.y * c);
    vec2 uv = fract(rotated / PERIOD) * PERIOD;

    vec2 f = flow(rotated / PERIOD);
